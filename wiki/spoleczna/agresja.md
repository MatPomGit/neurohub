...
Psychologowie wyróżniają kilka typologicznie istotnych podziałów:
...
Krytyka: model etologiczny słabo tłumaczy ogromne zróżnicowanie między kulturami w poziomie agresji.

## Hipoteza frustracji–agresji

Dollard, Doob, Miller, Mowrer i Sears (1939) sformułowali klasyczną hipotezę: **frustracja zawsze prowadzi do agresji, a agresja zawsze wynika z frustracji**.

Berkowitz (1962, 1989) zmodyfikował hipotezę:
- Frustracja prowadzi do gotowości agresywnej (negatywnego pobudzenia)
- Agresja jest wyzwalana przez **wskazówki agresywne** w otoczeniu
- Efekt broni (*weapons effect*): sama obecność broni w polu widzenia zwiększa agresywność

### Ogólny model agresji (Anderson & Bushman, 2002)
...
Dzieci obserwowały dorosłego modela, który agresywnie bił nadmuchiwaną lalkę Bobo. Następnie dzieci miały okazję bawić się z lalką:
- Dzieci, które widziały agresywny model, wykazywały **znacznie więcej agresji** wobec lalki niż dzieci z grupy kontrolnej
- Naśladowały specyficzne, wcześniej nieznane im gesty i słowa modela
- Efekt był silniejszy, gdy model był nagradzany za agresję
- Model w telewizji był równie skuteczny co model na żywo
...
- Motywacja (oczekiwanie nagrody lub kary)

## Media a agresja

### Badania korelacyjne i eksperymentalne

Metaanalizy (Anderson i in., 2010) wskazują na małą, ale konsistentną korelację między ekspozycją na przemoc w mediach (gry, filmy) a zachowaniami agresywnymi. Efekt jest silniejszy dla:
- Dzieci niż dorosłych
- Agresji normatywnej i dehumanizacji niż brutalnych przestępstw
- Krótkoterminowych niż długoterminowych efektów

### Kontrowersje

Czynnik konfundujący: dzieci z predyspozycjami do agresji mogą aktywniej szukać treści przemocy (selekcja mediów). Badania podłużne (Huesmann i in., 2003) sugerują jednak związek przyczynowy, kontrolując wcześniejszą agresywność.
...
| Czynnik | Rola |
|---|---|
| Testosteron | koreluje z dominacją i agresją reaktywną; związek niejednoznaczny przyczynowo |
| Serotonina | niski poziom wiązany z impulsywną agresją |
| Ciało migdałowate | szybka ocena zagrożenia; nadaktywność → agresja reaktywna |
| Kora przedczołowa | hamowanie impulsów; uszkodzenia → wzrost agresji |
| Geny | heritabilność agresji ~50% (badania bliźniąt); gen MAOA a podatność na agresję po maltretowaniu (Caspi, 2002) |

## Zarządzanie gniewem i redukcja agresji
