# Teoria systemów BIS/BAS Graya

Teoria Jeffreya Graya opisuje dwa podstawowe systemy motywacyjne mózgu — Behawioralny System Hamowania (BIS) i Behawioralny System Aktywacji (BAS) — które regulują reakcje na nagrodę i karę.

## Geneza teorii

Jeffrey Alan Gray (1934–2004) opracował swoją teorię jako **biologicznie ugruntowaną alternatywę** dla modelu Eysencka. Punktem wyjścia była krytyka hipotezy ARAS: Gray zaproponował, że różnice osobowościowe są lepiej wyjaśniane przez wrażliwość na sygnały nagrody i kary, nie przez ogólny poziom pobudzenia korowego.

## Behawioralny System Hamowania (BIS)

**BIS** (*Behavioural Inhibition System*) jest aktywowany przez:
- Sygnały kary (conditioned punishment)
- Sygnały braku nagrody (omission of expected reward)
- Nowość, nieznaność
- Bodźce awersywne i konflikty motywacyjne

### Efekty aktywacji BIS
- Zahamowanie trwającego zachowania (*behavioural inhibition*)
- Wzrost czujności i uwagi (*risk assessment*)
- Wzrost pobudzenia i napięcia
- Wzmożona analiza otoczenia pod kątem zagrożeń

### Neurobiologia BIS
BIS jest anatomicznie powiązany z:
- **Przegrodą i hipokampem** (system przegrodowo-hipokampalny, SEP)
- **Ciałem migdałowatym** (amygdala) — detekcja zagrożeń
- **Korą oczodołowo-czołową** — hamowanie zachowania

## Behawioralny System Aktywacji/Nagrody (BAS)

**BAS** (*Behavioural Activation/Approach System*) jest aktywowany przez:
- Sygnały nagrody (conditioned reward)
- Uwarunkowane sygnały nagrody
- Bodźce nagradzające bezwarunkowo

### Efekty aktywacji BAS
- Zbliżanie się do źródła nagrody
- Inicjowanie i kontynuowanie zachowania
- Pozytywne emocje (radość, podniecenie, euforia)
- Impulsywność przy bardzo wysokim poziomie aktywności

### Neurobiologia BAS
BAS jest powiązany z:
- ***Nucleus accumbens*** i prążkowiem brzusznym (układ mezokortykolimbiczny)
- **Dopaminą** — przekaźnik kluczowy dla oczekiwania nagrody
- **Korą przedczołową** — planowanie i inicjowanie działania

## Relacja BIS/BAS do wymiarów Eysencka

Gray sformułował **krytykę modelu EPQ** i zaproponował rotację osi osobowości Eysencka:

| Eysenck | Interpretacja Graya |
|---|---|
| Ekstrawersja ↔ Introwersja | wrażliwość na nagrodę (BAS) |
| Labilność ↔ Stabilność emocjonalna | wrażliwość na karę (BIS) |

Według Graya oś lęku (BIS) przebiega pod kątem 45° do osi neurotyzmu i introwersji Eysencka.

## Revised Reinforcement Sensitivity Theory (RST, 2004)

W 2004 roku Gray i McNaughton opublikowali **zrewidowaną RST**, która wyróżnia trzy systemy:

| System | Funkcja | Emocje |
|---|---|---|
| **BIS** | rozwiązywanie konfliktów motywacyjnych, czujność | lęk (*anxiety*) |
| **BAS** | zbliżanie się do nagród | nadzieja, radość, pożądanie |
| **FFFS** (*Fight-Flight-Freeze System*) | ucieczka przed zagrożeniem | strach (*fear*) |

Rozróżnienie **BIS (lęk) vs FFFS (strach)** jest kluczowe: BIS towarzyszy konflikty i niepewność, FFFS — bezpośrednie zagrożenia.

## BAS a impulsywność i uzależnienia

Wysokie BAS wiąże się z:
- **Impulsywnością** — skłonnością do szybkich decyzji napędzanych oczekiwaniem nagrody
- **Poszukiwaniem sensacji** (sensation seeking Zuckermana)
- Podatnością na **uzależnienia**: substancje psychoaktywne dostarczają silnego sygnału nagrodowego, nadmiernie aktywując BAS
- Hipomanią i symptomami ADHD

## BIS a lęk i depresja

Wysokie BIS wiąże się z:
- Ogólnym **zaburzeniem lękowym** (GAD) — tendencja do ruminacji i oceny ryzyka
- **Fobią społeczną** — silna wrażliwość na sygnały odrzucenia
- Podatnością na **depresję anaklitikoną** (przez chroniczne frustracje nagród)
- Perfekcjonizmem i skłonnością do prokrastynacji

## Pomiar BIS/BAS

**BIS/BAS Scales Carvera i Whitea (1994)** to najszerzej używane narzędzie: 20 pozycji, cztery skale:
- BIS (7 poz.) — lęk, wrażliwość na karę
- BAS Drive (4 poz.) — dążenie do celów
- BAS Fun Seeking (4 poz.) — spontaniczne poszukiwanie przyjemności
- BAS Reward Responsiveness (5 poz.) — pozytywne emocje na nagrodę

Narzędzie ma polskie adaptacje i jest szeroko stosowane w badaniach klinicznych.

## Literatura

- Gray, J. A. (1982). *The Neuropsychology of Anxiety*. Oxford University Press.
- Gray, J. A., & McNaughton, N. (2000). *The Neuropsychology of Anxiety* (2nd ed.). Oxford University Press.
- Carver, C. S., & White, T. L. (1994). Behavioral inhibition, behavioral activation, and affective responses to impending reward and punishment: The BIS/BAS Scales. *Journal of Personality and Social Psychology, 67*(2), 319–333.
- Corr, P. J. (2004). Reinforcement sensitivity theory and personality. *Neuroscience & Biobehavioral Reviews, 28*(3), 317–332.
- Smillie, L. D. (2008). What is reinforcement sensitivity? Neuroscience paradigms for approach-avoidance process theories of personality. *European Journal of Personality, 22*(5), 359–384.
- Corr, P. J. (Ed.). (2008). *The Reinforcement Sensitivity Theory of Personality*. Cambridge University Press.
