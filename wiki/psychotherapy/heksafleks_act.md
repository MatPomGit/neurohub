...
Heksafleks opisuje dwa koła sześciu procesów: koło **sztywności psychologicznej** (psychopathology hexaflex) i koło **elastyczności psychologicznej** (flexibility hexaflex). Każdemu procesowi dysfunkcjonalnemu odpowiada symetryczny proces terapeutyczny.

### Model patologii: Sześć procesów sztywności

| Proces sztywności | Opis kliniczny |
|-------------------|---------------|
| **Fuzja poznawcza** | Myśli traktowane jako literalne prawdy, a nie produkty umysłu; regulują zachowanie niezależnie od kontekstu |
| **Unikanie doświadczeń** | Chroniczne unikanie lub tłumienie negatywnych myśli, emocji, wspomnień, doznań ciała |
| **Dominacja konceptualizowanej przeszłości/przyszłości** | Umysł zaabsorbowany ruminacjami lub martwieniniem kosztem kontaktu z teraźniejszością |
| **Przywiązanie do konceptualizowanego Ja** | Sztywna narracyjna tożsamość ("Jestem lękowcem") broniąca się przed doświadczeniami z nią sprzecznymi |
| **Brak kontaktu z wartościami / wartości jako reguły** | Utrata kontaktu z tym, co naprawdę ważne; wartości instrumentalne lub brak wyartykułowanych wartości |
| **Nieaktywność, impulsywność, unikanie** | Brak zaangażowanego działania lub działania zdominowane przez unikanie emocjonalnych wyzwań |

Zjawisko **sztywności psychologicznej** (ang. *psychological inflexibility*) mierzone jest psychometrycznie Kwestionariuszem Akceptacji i Działania II (AAQ-II; Bond i in., 2011), który jest najszerzej stosowaną miarą tego konstruktu w literaturze ACT.
...
**Techniki defuzji:**
- Poprzedzanie każdej myśli formułą: "Zauważam, że mam myśl, że...";
- Wyobrażanie myśli jako liści płynących na rzece lub chmur dryfujących po niebie;
...
Jeden z najbardziej oryginalnych i filozoficznie zaawansowanych procesów modelu. Rozróżnia trzy poziomy poczucia siebie:

| Poziom Ja | Opis | Przykład |
|-----------|------|---------|
...
W modelu ACT **wartości** to *słownie skonstruowane globalne kierunki zachowania*, które są z natury niemożliwe do "osiągnięcia" na stałe i nieweryfikowalne empirycznie. Są podobne do ruchomego punktu na horyzoncie, który nadaje kierunek podróży.

**Kluczowe rozróżnienia:**
- Wartości ≠ cele: cel jest osiągalny i weryfikowalny (np. "ukończyć maraton"), wartość jest kierunkiem ciągłym (np. "dbać o zdrowie i witalność");
- Wartości ≠ odczucia: wartości pozostają ważne niezależnie od emocji towarzyszących ich realizacji;
- Wartości ≠ reguły: "powinienem być dobrym ojcem" (reguła oparta na lęku) vs. "zaangażowanie i obecność w relacji z dziećmi" (wartość).
...
| Obszar kliniczny | Poziom dowodów | Kluczowe metaanalizy |
|-----------------|---------------|---------------------|
| Ból przewlekły | Mocny (A) | Vowles i in. (2014), Fang & Ding (2020) |
| Depresja | Mocny (A) | A-Tjak i in. (2015), Twohig i in. (2020) |
| Zaburzenia lękowe (GAD, fobia) | Umiarkowany (B) | Arch i in. (2012), Swain i in. (2013) |
| OCD | Umiarkowany (B) | Bluett i in. (2014) |
| Psychoza | Umiarkowany (B) | Bach & Hayes (2002), Gaudiano & Herbert (2006) |
| Uzależnienia | Umiarkowany (B) | Lee i in. (2015) |
| Stres zawodowy / burnout | Umiarkowany (B) | Pross i in. (2014) |
| Zaburzenia odżywiania | Wstępny (C) | Hill i in. (2015) |
| PTSD | Wstępny (C) | Lang i in. (2012) |

### Mediatory terapeutycznych zmian

Badania procesów mediacyjnych wskazują, że:
- Elastyczność psychologiczna (mierzona AAQ-II) mediuje skuteczność ACT w bólu przewlekłym, depresji i lęku (Gloster i in., 2020);
- Defuzja poznawcza mediuje redukcję symptomów OCD niezależnie od ekspozycji (Twohig & Whittal, 2019);
- Klaryfikacja wartości mediuje efekty ACT w depresji nawet po kontroli akceptacji (Gloster i in., 2017).

## Heksafleks a inne modele terapeutyczne

### ACT a CBT (Beck)

| Wymiar | CBT (Beck) | ACT (Hayes) |
|--------|-----------|-------------|
| Cel wobec myśli | Zmiana treści (restrukturyzacja) | Zmiana funkcji (defuzja) |
...
DBT czerpie z tradycji zen (radykalna akceptacja) i dzieli z ACT nacisk na akceptację. Jednak DBT koncentruje się na regulacji emocji i uczeniu konkretnych umiejętności; Heksafleks ACT ma szerszy zakres procesów i silniejszy akcent na wartości jako motor zmiany.

### ACT a MBCT (Teasdale, Williams, Segal)

MBCT (Mindfulness-Based Cognitive Therapy) łączy CBT z formalnymi praktykami mindfulness w profilaktyce nawrotów depresji. ACT operacjonalizuje uważność w modelu Heksafleksu jako jeden z sześciu procesów, nie jako centralną interwencję.
...
### Heksafleks w kontekście traumy

W PTSD sztywność psychologiczna przejawia się jako unikanie doświadczeń traumatycznych (flashbacków, wspomnień, cielesnych reminiscencji), fuzja z przekonaniami "Jestem zniszczony/niewarty miłości", dominacja przeszłości. ACT nie stosuje klasycznej ekspozycji narracyjnej (jak EMDR czy prolonged exposure), lecz wspiera **akceptację obecności traumatycznych treści** i **ponowne zaangażowanie w życie wartościowe**. Badania Lang i in. (2012) i Twohig i in. (2020) wskazują na obiecujące wyniki, choć baza dowodowa jest mniejsza niż w przypadku EMDR i PE.
...
Organizational ACT (ACT@Work, Flaxman i in., 2013) stosuje procesy Heksafleksu w interwencjach antystresowych i antyburnout w środowisku pracy. Efekty obejmują redukcję absencji, poprawę dobrostanu pracowników i elastyczności behawioralnej.

### ACT w pediatrii i adolescencji

Zaadaptowane protokoły ACT dla dzieci i nastolatków (np. You Are Not Your Pain, Turrell & Bell, 2016) stosują metafory dostosowane do wieku, prace plastyczne i gry. Przegląd Swain i in. (2015) wskazuje na skuteczność w lęku szkolnym, bólu przewlekłym i OCD u młodzieży.

## Implikacje dla praktyki terapeutycznej

### Ocena procesów Heksafleksu w formułowaniu przypadku
...
Modele superwizji ACT (Luoma & Hayes, 2016) wykorzystują te same procesy do refleksji nad relacją terapeutyczną i trudnościami klinicznymi terapeuty: fuzja terapeuty z przekonaniami o kliencie, unikanie trudnych tematów w sesji, brak kontaktu z własnymi wartościami jako terapeuta.
...
- Bond, F. W., Hayes, S. C., Baer, R. A., Carpenter, K. C., Guenole, N., Orcutt, H. K., ... & Zettle, R. D. (2011). Preliminary psychometric properties of the Acceptance and Action Questionnaire–II. *Behavior Therapy, 42*(4), 676–688.
- Gloster, A. T., Walder, N., Levin, M., Twohig, M. P., & Karekla, M. (2020). The empirical status of acceptance and commitment therapy: A review of meta-analyses. *Journal of Contextual Behavioral Science, 18*, 181–192.
...
- Polk, K. L., & Schoendorff, B. (2014). *The ACT Matrix: A New Approach to Building Psychological Flexibility Across Settings and Populations*. New Harbinger.
- Rolffs, J. L., Rogge, R. D., & Wilson, K. G. (2018). Disentangling components of flexibility via the hexaflex model: Development and validation of the Multidimensional Psychological Flexibility Inventory (MPFI). *Assessment, 25*(4), 458–482.
