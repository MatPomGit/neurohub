...
## Hierarchia predykcji i błędy predykcji
...
Wynik? Percepcja jest dosłownie *najlepszą hipotezą*, jaką mózg potrafi wygenerować na podstawie wcześniejszej wiedzy (prior) i aktualnych sygnałów (likelihood).
...
Ważenie precyzji jest regulowane przez neuroprzekaźniki, szczególnie **dopaminę** (dla sygnałów zewnętrznych) i **acetylocholinę** (dla sygnałów z ciała).
...
|--------|------|---------|
| Aktualizacja przekonań | Zmień model świata | „Ta figura to sześcian, nie prostopadłościan" |
| Działanie | Zmień świat tak, by pasował do modelu | Sięgnij po szklankę, której się spodziewasz trzymać |

To radykalnie zmienia rozumienie ruchu: **motoryka to mechanizm spełniania predykcji proprioceptywnych**, a nie wynik obliczenia optymalnego toru ruchu.

## Percepcja: hierarchia a świadomość

PP oferuje eleganckie wyjaśnienie kilku zjawisk percepcyjnych:
...
W modelu PP depresja opisywana jest jako stan, w którym mózg generuje **pesymistyczne predykcje** (negatywne priors) z nadmierną pewnością:
- Świat jest przewidywalnie zły → ograniczenie eksploracji
- Behawioralna bezruchość minimalizuje ryzyko konfrontacji z błędami predykcji
- Terapia: interwencje zmieniające strukturę priors (CBT, aktywacja behawioralna)
...
2. Minimalizacji złożoności modelu (Occam's razor)

To pozwala opisać percepcję, działanie, uczenie się i ciekawość jako różne aspekty jednego matematycznego imperatywu.

## Krytyka i ograniczenia

| Zarzut | Kontrargument |
|--------|---------------|
| Teoria „wyjaśnia wszystko" → niefalsyfikowalna | FEP ma precyzyjne predykcje matematyczne, testowane w neuroimaging |
| Model jest zbyt abstrakcyjny | Implementacje obliczeniowe (POMDP, SPM) dają testowalne prognozy |
| Ignoruje rolę ciała i środowiska | Aktywne wnioskowanie explicite włącza działanie i embody cognition |
| Niejasne związki z subiektywnym przeżyciem (hard problem) | PP opisuje korelaty, nie rozwiązuje problemu hard problem |

## Zastosowania praktyczne i badania
...
| Termin | Definicja |
|--------|-----------|
| **Prior** | Wcześniejsze przekonanie o stanie świata |
| **Likelihood** | Prawdopodobieństwo sygnału sensorycznego dla danego stanu |
| **Posterior** | Zaktualizowane przekonanie (Bayes: prior × likelihood) |
| **Błąd predykcji (PE)** | Rozbieżność między predykcją a faktycznym sygnałem |
| **Precyzja** | Pewność co do sygnału, moduluje ważenie PE |
| **Wolna energia** | Miara rozbieżności model–rzeczywistość; jej minimalizacja ≈ dobre poznanie |
| **Aktywne wnioskowanie** | Działanie w celu spełnienia predykcji / redukcji PE przez zmianę środowiska |
...
- Murray, R. F., Chan, C., Hung, A., & Nguyen, T. (2014). *Bayesian approaches to perception and attention in autism.* **Frontiers in Human Neuroscience**, 8, 479.
